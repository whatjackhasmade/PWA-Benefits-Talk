...
Seperate from websites, these applications are often coded in languages fitting per device.
...
Mobile applications exist as reliable solutions for creating applications with capabilities to store large amounts of data and files offline.
...
Method of storing data locally like cookies, but for larger amounts of data.
...
An API to locate records either by their key using an index.

A query language can be used with the API.
...
---
## New Support
"Developers also have the option of proactively submitting PWAs for inclusion in the Microsoft Store.".
